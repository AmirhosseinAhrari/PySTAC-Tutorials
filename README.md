# PySTAC Tutorials

Practical tutorials on Earth observation satellite data analysis using Python, STAC, and xarray.

This repository accompanies the **PySTAC tutorial series** on my YouTube channel, [Google Earth Engine with Amirhossein Ahrari](https://www.youtube.com/@amirhosseinahrarigee).

The series covers accessing satellite imagery, calculating NDVI, analyzing monthly vegetation patterns, and exploring vegetation anomalies and variability.

## Tutorial series

| Tutorial | Topic | Video |
| --- | --- | --- |
| 1 | Sentinel-2 NDVI Analysis | [Watch tutorial](https://youtu.be/6nB0VSjVnAA) |
| 2 | Sentinel-2 Monthly NDVI Analysis | [Watch tutorial](https://youtu.be/o33od1LXcFs) |
| 3 | Sentinel-2 Land Cover and NDVI Analysis using Python | [Watch tutorial](https://youtu.be/PZfgMUvz6t4) |
| 4 | Sentinel-2 NDVI Anomaly Detection using Python | [Watch tutorial](https://youtu.be/YaA2Ug9Sgp4) |
| 5 | Sentinel-2 NDVI Coefficient of Variation (CV) Analysis using Python | [Watch tutorial](https://youtu.be/vTH_tM38A3s) |

## Repository status

The tutorial index is available here. The accompanying notebooks have not yet been uploaded.

## Learning topics

- Search for Sentinel-2 imagery through a STAC catalog.
- Work with satellite data in Python.
- Calculate and visualize NDVI.
- Explore monthly vegetation patterns.
- Use NDVI for land-cover analysis.
- Detect NDVI anomalies.
- Assess vegetation variability using the coefficient of variation.

## Author

**Amirhossein Ahrari**  
Post-Doctoral Researcher, University of Oulu, Finland  
Google Developer Expert in Google Earth Engine

- [YouTube](https://www.youtube.com/@amirhosseinahrarigee)
- [LinkedIn](https://www.linkedin.com/in/amirhosseinahrari/)
- [Google Earth Engine tutorials repository](https://github.com/AmirhosseinAhrari/GoogleEarthEngine)
